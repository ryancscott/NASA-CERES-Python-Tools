...
#
# This script matches FOVs from multiple products: CRS4, SSF4A, FF3C, FF4A soon...
#
...
crs_path = '/Users/rcscott2/Desktop/CRS/CRS_validation/_CRS4/' \
...
count = 0
# Now get footprints common to all products...
crs_data2 = []
ssf_data2 = []
ff3_data2 = []
# loop over sites
for site in site_names:
    ...
    crs_file = site + '_CRS4+OBS_' + satellite + '-' + flight_model + '_JAN-2019.txt'
    ssf_file = site + '_SSF4A+OBS_' + satellite + '-' + flight_model + '_JAN-2019.txt'
    ff3_file = site + '_FF3C+OBS_' + satellite + '-' + flight_model + '_JAN-2019.txt'
    ...
    # isolate FOV distance-to-site info... perhaps replace with Julian time?
    crs_dist = []
    ssf_dist = []
    ff3_dist = []
    for el in crs_data:
        crs_dist.append(el[5])
    for el in ssf_data:
        ssf_dist.append(el[5])
    for el in ff3_data:
        ff3_dist.append(el[5])

    # only use footprints when all 3 products have the
    # same computed dist to the site, i.e., footprints are the same
    intersection = set(crs_dist).intersection(ssf_dist, ff3_dist)
    num_common_fovs = len(intersection)
    print('# of common FOVs:', num_common_fovs)
    ...
    for crs_fov, ssf_fov, ff3_fov, inter in list(zip(crs_data, ssf_data, ff3_data, list(intersection))):
        if crs_fov[5] and ssf_fov[5] and ff3_fov[5] in list(intersection):
...
for el in ssf_data2:
    ssf_swd.append(el[9])
    ssf_lwd.append(el[11])
    # ssf_cf1.append(el[13])
    # ssf_cf2.append(el[14])

for el in ff3_data2:
    ff3_swd.append(el[9])
    ff3_lwd.append(el[11])
...
print('N_{CRS} = ', sum(~np.isnan(crs_lwd)))
print('N_{SSF} = ', sum(~np.isnan(ssf_lwd)))
print('N_{FF3} = ', sum(~np.isnan(ff3_lwd)))
print('N_{OBS} = ', sum(~np.isnan(obs_lwd)))
...
print('Bias:', crs_lw_mean_bias, ssf_lw_mean_bias, ff3_lw_mean_bias)
print('RMSD:', crs_lw_rms_diff, ssf_lw_rms_diff, ff3_lw_rms_diff)
print('Corr:', crs_lw_corr, ssf_lw_corr, ff3_lw_corr)
...
cbaxes = fig.add_axes([0.905, 0.53, 0.015, 0.35])
...
axs[0, 0].set_xlabel(r'Observed LW$\downarrow$ [W m$^{-2}$]')
axs[0, 0].grid()
...
axs[0, 1].set_xlabel(r'Observed LW$\downarrow$ [W m$^{-2}$]')
axs[0, 1].grid()
...
axs[0, 2].set_xlabel(r'Observed LW$\downarrow$ [W m$^{-2}$]')
axs[0, 2].grid()
axs[0, 2].set_axisbelow(True)
axs[0, 2].text(75, 370, ff3_lw_stats)

axs[1, 0].hist(crs_lwd-obs_lwd, bins=np.arange(-100, 100, 5), color='#A9CCE3')
axs[1, 0].set_xlim(-125, 125)
axs[1, 0].set_ylim(0, 100)
axs[1, 0].set_xlabel(r'Flux Difference [W m$^{-2}$]'+'\n'+'(CRS - OBS)')
axs[1, 0].set_ylabel('Number of CERES FOVs per Bin')
axs[1, 0].grid()
axs[1, 0].set_axisbelow(True)

axs[1, 1].hist(ssf_lwd-obs_lwd, bins=np.arange(-100, 100, 5), color='#A9CCE3')
...
axs[1, 2].hist(ff3_lwd-obs_lwd, bins=np.arange(-100, 100, 5), color='#A9CCE3')
...
print('SW Bias:', crs_sw_mean_bias, ssf_sw_mean_bias, ff3_sw_mean_bias)
print('SW RMSD:', crs_sw_rms_diff, ssf_sw_rms_diff, ff3_sw_rms_diff)
print('SW Corr:', crs_sw_corr, ssf_sw_corr, ff3_sw_corr)
...
axs[0, 0].plot(x, x, 'k')
_, _, _, im = axs[0, 0].hist2d(obs_swd, crs_swd, bins=[bins, bins], cmap='BuPu', vmin=0, vmax=10)
# fig.colorbar(im, ax=[axs[0, 0]], location='right')
...
axs[0, 0].set_xlabel(r'Observed SW$\downarrow$ [W m$^{-2}$]')
axs[0, 0].grid()
axs[0, 0].set_axisbelow(True)
axs[0, 0].text(80, 800, crs_sw_stats)

axs[0, 1].plot(x, x, 'k')
_, _, _, im = axs[0, 1].hist2d(obs_swd, ssf_swd, bins=[bins, bins], cmap='BuPu', vmin=0, vmax=10)
...
axs[0, 1].set_xlabel(r'Observed SW$\downarrow$ [W m$^{-2}$]')
axs[0, 1].grid()
axs[0, 1].set_axisbelow(True)
...
axs[0, 2].set_xlabel(r'Observed SW$\downarrow$ [W m$^{-2}$]')
axs[0, 2].grid()
axs[0, 2].set_axisbelow(True)
axs[0, 2].text(80, 800, ff3_sw_stats)


axs[1, 0].hist(crs_swd-obs_swd, bins=np.arange(-150, 150, 5), color='#A9CCE3')
...
axs[1, 1].hist(ssf_swd-obs_swd, bins=np.arange(-150, 150, 5), color='#A9CCE3')
...
axs[1, 2].hist(ff3_swd-obs_swd, bins=np.arange(-150, 150, 5), color='#A9CCE3')
